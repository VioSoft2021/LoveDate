-- 2026-05-31 Stability Assessment — optional SECOND assessment
--
-- Adds the discoverable projection for the new (opt-in) Relationship
-- Stability Assessment. Raw 12-item Likert answers live in profile_private
-- (owner-only RLS); sync_discoverable_profile derives the public
-- stability_profile jsonb (3 trait scores 0..100 + value stances) and writes
-- it to public.profiles so it can power stability-aware matching without
-- leaking raw answers. Mirrors the 2026-05-24 Tier A migration.
--
-- Idempotent. The function is re-created in full (create or replace), so the
-- Tier A behavior is preserved verbatim and only the stability branch is new.

set search_path = public;

-- ── Private storage: raw stability answers + completion timestamp ──────
alter table public.profile_private
  add column if not exists stability_answers integer[] default null,
  add column if not exists stability_completed_at timestamptz default null;

comment on column public.profile_private.stability_answers is
  'Stability Assessment raw Likert answers (12 integers, each 1..5). NEVER readable to other users (RLS owner-only).';
comment on column public.profile_private.stability_completed_at is
  'ISO timestamp of most-recent Stability Assessment completion.';

-- ── Public derived field: the stability profile ──────────────────────
alter table public.profiles
  add column if not exists stability_profile jsonb default null;

comment on column public.profiles.stability_profile is
  'Public stability profile derived server-side from the 12 stability answers. Shape: {conflictRepair,commitment,communication (each 0..100), values:{children,finances,pace}, completedAt}. NULL until the user takes the optional Stability Assessment.';

-- ── sync_discoverable_profile: append the stability branch ────────────
...
begin
  ...
  insert into public.profiles (
    ...
    big_five, attachment_style, stability_profile
  )
  values (
    ...
    v_big_five, v_att_style, v_stability
  )
...
-- ── Server-side cache for the per-match AI stability reveal ───────────
-- Mirrors pair_dynamic_cache: both members of a pair compute the same
-- cache_key, so once one pays the Claude tokens the other's view is served
-- from cache. Written only by the Edge Function via the service-role key
-- (which bypasses RLS), so RLS is enabled with no policies — locked to
-- clients, open to the function.
create table if not exists public.stability_reveal_cache (
...
alter table public.stability_reveal_cache enable row level security;

comment on table public.stability_reveal_cache is
  'Per-pair cache of the AI stability reveal (30-day TTL enforced in the Edge Function). Service-role write only; RLS denies all client access.';
